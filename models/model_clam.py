...
class CLAM_SB(nn.Module):
    def __init__(self, gate=True, size_arg="small", dropout=0., k_sample=8,
                 n_classes=28, instance_loss_fn=nn.BCEWithLogitsLoss(),
                 subtyping=False, embed_dim=1024):
        super().__init__()
        # Define dimensions for small/big settings
        self.size_dict = {"small": [embed_dim, 512, 256],
                          "big": [embed_dim, 512, 384]}
        size = self.size_dict[size_arg]
        # Shared feature encoder + attention
        fc = [nn.Linear(size[0], size[1]), nn.ReLU(), nn.Dropout(dropout)]
        if gate:
            attention_net = Attn_Net_Gated(L=size[1], D=size[2], dropout=dropout, n_classes=1)
        else:
        ...
        fc.append(attention_net)
        self.attention_net = nn.Sequential(*fc)
        # Bag-level classifier: multi-label via sigmoid
        self.classifiers = nn.Linear(size[1], n_classes)
        # Instance-level classifiers per class
    ...
    def forward(self, h, label=None, instance_eval=False,
        ...
        # Attention pooling
        A, h = self.attention_net(h)      # NxK
        A = A.transpose(1, 0)             # KxN
        if attention_only:
            return A
        A_raw = A
        A = F.softmax(A, dim=1)          # normalize across N

        # Instance-level loss for multi-label
        if instance_eval:
            total_inst_loss = 0.0
            inst_preds, inst_targets = [], []
            inst_labels = label.long()     # shape: (batch_size, n_classes)
            for i, classifier in enumerate(self.instance_classifiers):
                # Check if any positive instance in class i
                pos_mask = inst_labels[:, i].sum() if inst_labels.dim() > 1 else inst_labels.sum()
                if pos_mask.item() > 0:
                    loss_i, preds_i, targets_i = self.inst_eval(A[i], h, classifier)
                elif self.subtyping:
                    loss_i, preds_i, targets_i = self.inst_eval_out(A[i], h, classifier)
                else:
                    continue
                total_inst_loss += loss_i
                inst_preds.extend(preds_i.cpu().numpy())
                inst_targets.extend(targets_i.cpu().numpy())
            if self.subtyping and len(self.instance_classifiers) > 0:
                total_inst_loss /= len(self.instance_classifiers)

        # Bag-level inference
        M = torch.mm(A, h)
        logits = self.classifiers(M)      # shape: (batch_size, n_classes)
        Y_prob = torch.sigmoid(logits)    # multi-label probabilities
        Y_hat = (Y_prob > 0.5).float()    # multi-label predictions
        ...
        if instance_eval:
            results_dict = {
                ...
                'inst_preds': np.array(inst_preds),
                'inst_labels': np.array(inst_targets)
            }
        if return_features:
